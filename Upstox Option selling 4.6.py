...
def price_strike(expiry, price, option):
    if 'BANK' in expiry:
        try:
...
def close_and_fresh(c):
    if trade_table[c]['strategy'][0] == 'D20ironfly' or trade_table[c]['strategy'][2] == 'D20ironfly' or trade_table[c]['strategy'][0] == 'FIX.PRO':
...
def fixed_profit():
    global trade_table, flag, Trade, strategy, SL_price, Target1, qty, SL_qty, order_rej
    global order_id, tradingsymbol, rank, strike, days_left, order_placed, SL_D_price, Entry_bid
    global prem, Entry_status, Tgt1_status, SL_status, product, last_tgt, last_qty, week1b
    print('Fixprofit requirement check')
    for c in range(len(client)):
        if len(trade_table[c]) != 0:
            flag['call_delta'][c] = flag['put_delta'][c] = 0.0
            flag['scall_delta'][c] = flag['sput_delta'][c] = 0.0
            for i in range(len(trade_table[c])):
                if 'c' in trade_table[c]['rank'][i] and trade_table[c]['Entry_status'][i] == 'Executed' and client['fixprofit'].iloc[c] > 0:
                    if trade_table[c]['trade_type'][i] == 'SELL @':
                        flag['call_delta'][c] += float(trade_table[c]['delta'][i] * trade_table[c]['qty'][i] / client['fixprofit'].iloc[c] / 15)
                    else:
                        flag['call_delta'][c] -= float(trade_table[c]['delta'][i] * trade_table[c]['qty'][i] / client['fixprofit'].iloc[c] / 15)
                elif 'p' in trade_table[c]['rank'][i] and trade_table[c]['Entry_status'][i] == 'Executed' and client['fixprofit'].iloc[c] > 0:
                    if trade_table[c]['trade_type'][i] == 'SELL @':
                        flag['put_delta'][c] += float(trade_table[c]['delta'][i] * trade_table[c]['qty'][i] / client['fixprofit'].iloc[c] / 15)
                    else:
                        flag['put_delta'][c] -= float(trade_table[c]['delta'][i] * trade_table[c]['qty'][i] / client['fixprofit'].iloc[c] / 15)
            if client['fixprofit'].iloc[c] > 0 and time(9, 20) <= datetime.now().time() <= time(15, 20):
                try:
                    ind20p = (trade_table[c][trade_table[c]['3pm_tgt'] == 'OPEN']
                        [trade_table[c]['Entry_status'] == 'Executed']
                        [trade_table[c]['rank'] == 'put1']).index[0]
                    # ind07p = (trade_table[c][trade_table[c]['3pm_tgt'] == 'OPEN']
                    #     [trade_table[c]['Entry_status'] == 'Executed']
                    #     [trade_table[c]['rank'] == 'D07p']).index[0]
                    # if (trade_table[c]['strike'][ind20p] - trade_table[c]['strike'][ind07p]) <= 100:
                    ...
                    ind20c = (trade_table[c][trade_table[c]['3pm_tgt'] == 'OPEN']
                        [trade_table[c]['Entry_status'] == 'Executed']
                        [trade_table[c]['rank'] == 'call1']).index[0]
                    # ind07c = (trade_table[c][trade_table[c]['3pm_tgt'] == 'OPEN']
                    #     [trade_table[c]['Entry_status'] == 'Executed']
                    #     [trade_table[c]['rank'] == 'D07c']).index[0]
                    # if (trade_table[c]['strike'][ind07c] - trade_table[c]['strike'][ind20c]) <= 100:
                    ...
                    try:
                        ...
                        if flag['closing_profit'][c] == 0:
                            ...
                            if flag['profit'][c] > flag['closing_profit'][c]:
                                flag['closing_profit'][c] = flag['profit'][c]
                            elif flag['profit'][c] < flag['closing_profit'][c] - (client['fixprofit'].iloc[c] * 50):
                                close_and_fresh(c)
                                fixed_profit_entry()
                    except Exception as e:
                        print('Call is OK', c, e)
                    try:
                        ...
                        print('Put is OK', c, e)
                if ((flag['call_delta'][c] + flag['put_delta'][c]) > 0.2 or (trade_table[c]['LTP'][ind20c] > trade_table[c]['LTP'][ind20p] * 3) or (trade_table[c]['LTP'][ind20p] > trade_table[c]['LTP'][ind20c] * 3)):
                    print('Delta is ', flag['call_delta'][c] + flag['put_delta'][c])
                    try:
                        # indp = (trade_table[c][trade_table[c]['3pm_tgt'] == 'OPEN']
                        #     [trade_table[c]['Entry_status'] == 'Executed']
                        #     [trade_table[c]['rank'] == 'put1']).index[0]
                        pprice = trade_table[c]['LTP'][ind20p]
                        cprice = trade_table[c]['LTP'][ind20c]
                        total = pprice + cprice
                        reentry_price = total/2
                        expiry = week1b
                        order_id1=u[c].place_order(TransactionType.Buy,u[c].get_instrument_by_symbol(exchange,trade_table[c]['symbol'][ind20p]),
                            int(trade_table[c]['qty'][ind20p]),OrderType.Market,ProductType.Delivery,
                            0.0,None,0,DurationType.DAY,None,None,None)
                        ...
                        trade_table[c]['SL_qty'][ind20p] = trade_table[c]['Profit'][ind20p] = 0
                        trade_table[c]['R.Profit'][ind20p] = trade_table[c]['SL'][ind20p]
                        strike, tradingsymbol = price_strike(expiry, reentry_price, 'put')
                        rank = 'put1'
                        Trade = 'SELL @'
                        strategy = trade_table[c]['strategy'][ind20p]
                        order_placed = 0
                        SL_D_price = 'Nil'
                        prem = 0
                        Target1 = 10
                        ...
                        data = u[c].get_live_feed(u[c].get_instrument_by_symbol('NSE_FO', tradingsymbol), LiveFeedType.Full)
                        Entry_bid = data['bids'][0]['price'] + 0.05
                        qty = SL_qty = int(client['fixprofit'].iloc[c] * 15)
                        order_id1=u[c].place_order(TransactionType.Sell,u[c].get_instrument_by_symbol(exchange,tradingsymbol),
                            qty,OrderType.Market,ProductType.Delivery,
                            0.0,None,0,DurationType.DAY,None,None,None)
                        order_id = order_id1['order_id']
                        ...
                        qty = SL_qty = int(trade_table[c]['qty'][ind20c])
                        rank = 'call1'
                        strike, tradingsymbol = price_strike(expiry, reentry_price, 'call')
                        order_id1=u[c].place_order(TransactionType.Sell,u[c].get_instrument_by_symbol(exchange,tradingsymbol),
                            qty,OrderType.Market,ProductType.Delivery,
                            0.0,None,0,DurationType.DAY,None,None,None)
                        order_id = order_id1['order_id']
...
def fixed_profit_entry():
    ...
    days_left = fromtime1
    for c in range(len(client)):
        if client['fixprofit'].iloc[c] >= 1 and client['banknifty'].iloc[c] == 1:
            try:
                ind = (trade_table[c][trade_table[c]['3pm_tgt'] == 'OPEN']
                    [trade_table[c]['strategy'] == 'FIX.PRO']
                    [trade_table[c]['Entry_status'] == 'Executed']).index[0]
            except Exception as e:
                print(e, 'Not found existing trade for Fixed Profit')
                print('First trade for Fixed Profit')
                strike, tradingsymbol = price_strike(expiry, 60, 'call')
                rank = 'call1'
                data = u[c].get_live_feed(u[c].get_instrument_by_symbol('NSE_FO', tradingsymbol), LiveFeedType.Full)
                Entry_bid = data['asks'][0]['price'] - 0.05
                qty = SL_qty = int(client['fixprofit'].iloc[c] * 15)
                ...
                order_id = order_id1['order_id']
                append_excel_table(c)
                strike, tradingsymbol = price_strike(expiry, 60, 'put')
                rank = 'put1'
                data = u[c].get_live_feed(u[c].get_instrument_by_symbol('NSE_FO', tradingsymbol), LiveFeedType.Full)
                Entry_bid = data['asks'][0]['price'] - 0.05
                qty = SL_qty = int(client['fixprofit'].iloc[c] * 15)
...
if question.lower() == 'c':
    ...
    if time(6, 16) < datetime.now().time() < time(16, 35):
        ...
        schedule.every().day.at("09:18:02").do(close_stbt)
        ...
        schedule.every().day.at("09:17:13").do(fixed_profit_entry)
        schedule.every().day.at("09:19:02").do(new_position)
    ...
    if time(5, 35) < datetime.now().time() < time(15, 35):
        ...
        schedule.every().day.at("09:17:11").do(bankfuture_15)
        schedule.every().day.at("09:17:12").do(bankfuture_60)
        schedule.every().day.at("09:18:13").do(fixed_profit_entry)
